...
import com.example.xavier.myapplication.backend.MesureBean;
...
public class Map extends AppCompatActivity implements OnMapReadyCallback {
    ...
    private void putSavedLocations() {
        new AsyncTask<Void, Void, List<MesureBean>>() {
            @Override
            protected List<MesureBean> doInBackground(Void... params) {
                ...
                try {
                    return api.getMesure().execute().getItems();
                } catch (IOException e) {
                    e.printStackTrace();
                }
                return null;
            ...
            @Override
            protected void onPostExecute(final List<MesureBean> mesureB) {
                super.onPostExecute(mesureB);
                if (mesureB != null)
                    map.getMapAsync(new OnMapReadyCallback() {
                        @Override
                        public void onMapReady(GoogleMap googleMap) {
                            for (MesureBean mesureBean : mesureB) {
                                googleMap.addMarker(new MarkerOptions()
                                        .position(mesureB.getLocationAsLatLng(mesureBean.getLocation()))
                                        .icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_BLUE)));
                            }
                        }
                    });
