...
public class Mesure extends AppCompatActivity implements ConnectionCallbacks,
    ...
    private void saveEvent(Mesure mesure) {
        new AsyncTask<MesureBean, Void, Void>() {
            @Override
            protected Void doInBackground(MesureBean... params) {
                ...
                try {
                    api.storeMesure(params[0]).execute();
                } catch (IOException e) {
                    e.printStackTrace();
                }
                return null;
            }
        }.execute(Mesure.mesureToMesureBean(mesure));
    }
}